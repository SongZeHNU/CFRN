...
class freEncoding(nn.Module):
    def __init__(self, 
        ...
        if require_grad:
            self.emb = nn.Parameter(self.emb, requires_grad=True)    
            #self.fai = nn.Parameter(torch.zeros(1,xx, out_dim//2), requires_grad=True)    
        self.in_dim = in_dim
        self.out_dim = out_dim
        self.sigma = sigma
        self.cat_input = cat_input
        self.require_grad = require_grad
        self.conv1 = nn.Linear(out_dim//2,out_dim//2)
        self.conv2 = nn.Linear(out_dim//2,out_dim//2)

    def forward(self, x):
       
        if not self.require_grad:
            self.emb = self.emb.to(x.device)
            self.fai = self.fai.to(x.device)
        
        #f1 = self.conv1(x)
        y = torch.matmul(x, self.emb.T)
        #amp = self.conv2(y)
        if self.cat_input:
            return torch.cat([x, torch.sin(y + self.fai), torch.cos(y + self.fai)], dim=-1)
        else:
            return torch.cat([torch.sin(y), torch.cos(y)], dim=-1)

class resEncoding(nn.Module):
    def __init__(self, 
                 in_dim, 
                 out_dim, 
                 sigma = 6,
        ...
        self.cat_input = cat_input
        self.require_grad = require_grad
        self.conv1 = nn.Linear(32,32, bias=False)
        self.conv2 = nn.Linear(32,32, bias=False)
        self.fc = nn.Linear(12,32, bias=False)
    ...
    def forward(self, x, feat):
        ...
        y = torch.matmul(x, self.emb.T)
        #frey_fc = self.fc(y)
        frey = torch.cat([torch.sin(y), torch.cos(y)], dim=-1)
        frey_fc = self.fc(frey)
        #amp = self.conv1(feat)
        #res = self.conv2(feat)
        return self.conv1((feat+frey_fc))
    

class frefaiEncoding(nn.Module):
    def __init__(self, 
                 in_dim, 
                 out_dim, 
                 size,
        ...
        if require_grad:
            self.emb = nn.Parameter(self.emb, requires_grad=True)    
            self.fai = nn.Parameter(torch.zeros(1,size*size, out_dim//2), requires_grad=True)    
        self.in_dim = in_dim
        self.out_dim = out_dim
        self.sigma = sigma
        self.cat_input = cat_input
...
def ifa_frefeat(res, amp, size, stride=1, local=False):
    bs, hh, ww = res.shape[0], res.shape[-2], res.shape[-1]
    h , w = size
    coords = (make_coord((h,w)).cuda().flip(-1) + 1) / 2              #coords info (normalized to [-1,1])
    coords = coords.unsqueeze(0).expand(bs, *coords.shape)
    coords = (coords*2-1).flip(-1)


    bs, q_sample, _ = coords.shape
    ...
    for vx in vx_list:
        for vy in vy_list:
            ...
            q_feat = F.grid_sample(res, coords_.flip(-1).unsqueeze(1),mode='nearest',align_corners=False)[:,:,0,:].permute(0,2,1)       #nearest

            q_amp = F.grid_sample(amp, coords_.flip(-1).unsqueeze(1),mode='nearest',align_corners=False)[:,:,0,:].permute(0,2,1)       #nearest
            q_coord = F.grid_sample(feat_coords, coords_.flip(-1).unsqueeze(1),mode='nearest',align_corners=False)[:,:,0,:].permute(0,2,1)  #bilinear
            rel_coord = coords - q_coord
            rel_coord[:,:,0] *= hh#res.shape[-2]
            rel_coord[:,:,1] *= ww#res.shape[-1]

            bs, q = coords.shape[:2]
            q_freq = torch.stack(torch.split(q_feat, 2, dim=-1), dim=-1)
            ...
            inp = torch.mul(q_amp, q_freq) 
            #inp = q_freq
            out = torch.cat([inp, q_feat],dim=-1)
            if local:
